    ...
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-200px)] max-w-3xl mx-auto px-4">
      <div className="text-center mb-12">
        ...
        <p className="text-muted-foreground text-lg">
          {state === 'idle' && (hasInteracted ? 'Click the microphone or say "Hey MIKE"' : 'Click the microphone to start')}
          {state === 'listening' && <span className="text-accent animate-pulse">🎤 Listening to you...</span>}
          {state === 'processing' && <span className="text-primary animate-pulse">⚡ Processing request...</span>}
          {state === 'speaking' && <span className="text-green-400 animate-pulse">🔊 Responding...</span>}
        </p>
      ...
      <div className="relative w-72 h-72 mb-12 flex items-center justify-center">
        ...
        }`} style={{ animation: state !== 'idle' ? 'ringPulse 2s infinite' : 'none', animationDelay: '0s' }} />
        ...
        }`} style={{ animation: state !== 'idle' ? 'ringPulse 2s infinite' : 'none', animationDelay: '0.3s' }} />
        ...
        }`} style={{ animation: state !== 'idle' ? 'ringPulse 2s infinite' : 'none', animationDelay: '0.6s' }} />
        <div className={`absolute inset-12 rounded-full transition-all duration-500 ${
          state === 'idle' ? 'bg-gradient-to-br from-accent/20 to-primary/10' :
          state === 'listening' ? 'bg-gradient-to-br from-accent/50 to-accent/20 animate-corePulse' :
          state === 'processing' ? 'bg-gradient-to-br from-primary/50 to-accent/30 animate-coreSpin' :
        ...
        <div className={`absolute w-4 h-4 rounded-full transition-all duration-300 ${
          state === 'idle' ? 'bg-accent/50' :
          state === 'listening' ? 'bg-accent shadow-lg shadow-accent/50 animate-pulse' :
          state === 'processing' ? 'bg-white shadow-lg shadow-white/50 animate-spin' :
        ...
        <div className="absolute inset-0 flex items-center justify-center gap-1 px-20">
          {[...Array(19)].map((_, i) => (
            <div key={i} className="w-1 rounded-full transition-all duration-200" style={{
              ...
              backgroundColor: state === 'idle' ? 'rgba(0,200,255,0.3)' : state === 'speaking' ? `hsl(${160 + i * 5}, 80%, 60%)` : `hsl(${200 + i * 3}, 80%, 55%)`,
      ...
      {response && (
        <div className="glass-panel p-6 w-full mb-6 border border-green-500/30 animate-fade-in">
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">MIKE</p>
          <p className="text-foreground leading-relaxed">{response}</p>
        </div>
      )}

      <div className="flex items-center gap-8 mb-12">
        <button onClick={handleMicClick} className={`relative w-20 h-20 rounded-full transition-all duration-300 hover:scale-110 active:scale-95 ${
          state === 'idle' ? 'bg-gradient-to-br from-accent/30 to-primary/20 border-2 border-accent shadow-lg shadow-accent/20' :
          state === 'listening' ? 'bg-gradient-to-br from-accent/60 to-accent/40 border-2 border-accent shadow-xl shadow-accent/40 animate-pulse' :
          state === 'processing' ? 'bg-gradient-to-br from-primary/60 to-primary/40 border-2 border-primary shadow-xl shadow-primary/40' :
      ...
      <div className="w-full mb-6">
        <div className="glass-panel p-4 border border-border/30">
          ...
          <div className="relative">
            <button onClick={() => setShowDropdown(!showDropdown)} className="w-full flex items-center justify-between bg-card/40 border border-border/30 rounded-lg px-4 py-3 hover:border-accent/50 transition-all">
            ...
            {showDropdown && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-background/95 backdrop-blur-xl border border-border/50 rounded-xl overflow-hidden z-50 shadow-2xl">
                {VOICE_OPTIONS.map(voice => (
                  <button key={voice.id} onClick={() => selectAccent(voice.id)} className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-accent/10 transition-all ${selectedAccent === voice.id ? 'bg-accent/10 border-l-2 border-accent' : ''}`}>
                    <span className="text-xl">{voice.accent}</span>
                    <div className="text-left flex-1">
                      <p className="text-sm font-medium text-foreground">{voice.name}</p>
                      <p className="text-xs text-muted-foreground">{voice.description}</p>
                    </div>
                    {selectedAccent === voice.id && <Check className="w-4 h-4 text-accent" />}
                  </button>
                ))}
              </div>
            )}
      ...
      <div className="glass-panel p-6 w-full border border-border/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {tips.map((tip) => (
            <div key={tip.cmd} onClick={() => { setHasInteracted(true); speak(tip.cmd); }} className="p-3 rounded-lg bg-card/30 border border-border/30 hover:border-accent/50 hover:bg-card/50 cursor-pointer transition-all group">
              <p className="text-sm text-foreground/70 group-hover:text-accent transition-colors">{tip.icon} {tip.cmd}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        .animate-corePulse { animation: corePulse 1.5s ease-in-out infinite; }
        .animate-coreSpin { animation: coreSpin 4s linear infinite; }
        .animate-spin-slow { animation: coreSpin 8s linear infinite; }
        .text-gradient {
